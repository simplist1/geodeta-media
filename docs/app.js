...
function saveState(){
  ...
  try{ localStorage.setItem(STORAGE_KEY, JSON.stringify(safe)); }catch(error){ showToast('Local storage is full'); }
}
...
function episodeMarkup(ep){
  ...
  return `<article class="episode" data-id="${esc(ep.id)}">
    ...
    <div class="episode-main">
      ...
      <div><p class="podcast-name">${esc(ep.tag || 'Episode')}</p><p class="episode-title">${esc(ep.title)}</p>
        <div class="meta"><span class="source-badge"><i data-lucide="${source.icon}"></i>${source.label}</span><span>${esc(ep.time || 'Not started')}</span><div class="progress"><span style="width:${Math.max(0,Math.min(100,Number(ep.progress)||0))}%"></span></div></div>
...
function wireEpisodes(container){
  container.querySelectorAll('.episode').forEach(card=>{
    ...
    card.querySelector('.episode-delete').addEventListener('click',event=>{ event.stopPropagation(); state.episodes=state.episodes.filter(ep=>ep.id!==id); saveState(); renderAll(); showToast('Episode removed'); });
...
function renderRecent(){ const recent=[...state.episodes].sort((a,b)=>b.savedAt-a.savedAt).slice(0,5); $('#recentEpisodes').innerHTML=recent.map(episodeMarkup).join(''); $('#recentEmpty').hidden=recent.length>0; wireEpisodes($('#recentEpisodes')); }
...
function renderCollections(){
  ...
  $('#groups').innerHTML=list.map(c=>`<article class="group-card" data-id="${esc(c.id)}" draggable="true">
    <div class="card-tools"><button class="round-tool drag-handle" aria-label="Move collection"><i data-lucide="grip-vertical"></i></button><button class="round-tool collection-edit" aria-label="Edit collection"><i data-lucide="pencil"></i></button><button class="round-tool collection-delete" aria-label="Delete collection"><i data-lucide="x"></i></button></div>
    <span class="group-icon" style="background:${esc(c.color)}"><i data-lucide="${esc(c.icon)}"></i></span><strong>${esc(c.name)}</strong><span class="count">${countFor(c.id)} saved</span>
  </article>`).join('');
  $$('.group-card').forEach(card=>{
    ...
    card.querySelector('.collection-delete').addEventListener('click',event=>{ event.stopPropagation(); state.collections=state.collections.filter(c=>c.id!==id); state.episodes.forEach(ep=>ep.groups=ep.groups.filter(group=>group!==id)); saveState(); renderAll(); showToast('Collection removed'); });
...
function openCollection(id){
  activeCollection=state.collections.find(c=>c.id===id); if(!activeCollection)return;
  ...
  $('#episodeSearch').value=''; $('#showSpotify').checked=true; $('#showLocal').checked=true; $('#showOnline').checked=true;
  renderEpisodes(); showView($('#collectionView'));
}
function renderEpisodes(){
  if(!activeCollection)return;
  const term=$('#episodeSearch').value.trim().toLowerCase();
  const allowed={spotify:$('#showSpotify').checked,local:$('#showLocal').checked,online:$('#showOnline').checked};
  let list=activeCollection.id==='all' ? state.episodes : state.episodes.filter(ep=>ep.groups.includes(activeCollection.id));
  list=list.filter(ep=>(`${ep.tag} ${ep.title}`).toLowerCase().includes(term)&&allowed[ep.source]);
  $('#episodes').innerHTML=list.map(episodeMarkup).join(''); $('#emptyCollection').hidden=list.length>0; wireEpisodes($('#episodes'));
  $('#filterButton').classList.toggle('active',!Object.values(allowed).every(Boolean));
}
...
function openCollectionSheet(id=null){
  ...
  selectedIcon=collection?.icon||'library'; selectedColor=collection?.color||colors[0];
  $('#collectionSheetTitle').textContent=collection?'Edit collection':'New collection'; $('#createCollection').textContent=collection?'Save changes':'Create collection';
  $('#collectionName').value=collection?.name||''; $('#iconSearch').value=''; renderIconGrid(); renderColors(); openSheet('#collectionSheet');
  setTimeout(()=>$('#collectionName').focus(),180);
}
function saveCollection(){
  const name=$('#collectionName').value.trim(); if(!name){ showToast('Enter a collection name'); return; }
  if(editingCollectionId){ const collection=state.collections.find(c=>c.id===editingCollectionId); if(collection)Object.assign(collection,{name,icon:selectedIcon,color:selectedColor}); showToast('Collection updated'); }
  else{ state.collections.push({id:crypto.randomUUID(),name,icon:selectedIcon,color:selectedColor}); showToast('Collection created'); }
  saveState(); renderAll(); closeSheets();
}

function setSource(source){
  addSource=source; $$('.source-tab').forEach(button=>button.classList.toggle('selected',button.dataset.source===source));
  $('#spotifyFields').hidden=source!=='spotify'; $('#localFields').hidden=source!=='local'; $('#onlineFields').hidden=source!=='online';
  $('#spotifyArtworkButton').hidden=source!=='spotify'; refreshIcons();
}
...
function spotifyFallbackArtwork(){
  const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="300" height="300"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#1ed760"/><stop offset="1" stop-color="#0c7c3b"/></linearGradient></defs><rect width="300" height="300" rx="45" fill="url(#g)"/><circle cx="150" cy="150" r="92" fill="#101010"/><path d="M94 125c39-15 86-12 123 8M101 153c34-11 76-8 108 8M109 180c27-7 59-5 84 7" fill="none" stroke="#1ed760" stroke-width="14" stroke-linecap="round"/></svg>`;
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg)}`;
}
async function syncSpotifyArtwork(silent=false){
  const url=$('#spotifyUrl').value.trim(); if(!url||!url.includes('spotify.com')){ if(!silent)showToast('Enter a Spotify link first'); return; }
  $('#spotifyArtworkButton').disabled=true; $('#spotifyArtworkButton').innerHTML='<i data-lucide="loader-circle"></i> Syncing artwork'; refreshIcons();
  try{
    const response=await fetch(`https://open.spotify.com/oembed?url=${encodeURIComponent(url)}`);
    if(!response.ok)throw new Error('Artwork request failed');
    const data=await response.json(); if(!data.thumbnail_url)throw new Error('No thumbnail');
    draftArtwork=data.thumbnail_url; draftArtworkSource='spotify'; updateArtworkPreview(); if(!silent)showToast('Spotify artwork synced');
  }catch(error){
    if(draftArtworkSource!=='custom'){ draftArtwork=spotifyFallbackArtwork(); draftArtworkSource='spotify'; updateArtworkPreview(); }
    if(!silent)showToast('Using Spotify fallback artwork');
  }finally{ $('#spotifyArtworkButton').disabled=false; $('#spotifyArtworkButton').innerHTML='<i data-lucide="refresh-cw"></i> Sync Spotify artwork'; refreshIcons(); }
}
...
function extractSpotifyEmbed(url){ try{ const parsed=new URL(url); const parts=parsed.pathname.split('/').filter(Boolean); const type=parts[0],id=parts[1]; return type&&id?`https://open.spotify.com/embed/${type}/${id}`:''; }catch{return '';} }
function openEpisodeSheet(id=null){
  editingEpisodeId=id; const ep=id?state.episodes.find(item=>item.id===id):null;
  $('#episodeSheetTitle').textContent=ep?'Edit episode':'Add podcast media'; $('#saveEpisode').textContent=ep?'Save changes':'Add episode';
  $('#episodeTitle').value=ep?.title||''; $('#episodeTag').value=ep?.tag||''; $('#spotifyUrl').value=ep?.url||''; $('#onlineName').value=ep?.onlineName||''; $('#onlinePath').value=ep?.onlinePath||''; $('#audioFile').value=''; $('#artworkFile').value='';
  draftArtwork=ep?.artImage||''; draftArtworkSource=ep?.artSource||'default'; draftLocalUrl=ep?.localUrl||''; updateArtworkPreview(); setSource(ep?.source||'spotify'); openSheet('#episodeSheet');
}
async function saveEpisode(){
  const title=$('#episodeTitle').value.trim()||'Untitled episode'; const tag=$('#episodeTag').value.trim()||sourceInfo(addSource).label;
  const existing=editingEpisodeId?state.episodes.find(ep=>ep.id===editingEpisodeId):null;
  const target=existing||{id:crypto.randomUUID(),groups:activeCollection&&activeCollection.id!=='all'?[activeCollection.id]:['all'],progress:0,savedAt:Date.now(),artClass:addSource==='online'?'three':addSource==='local'?'five':'one'};
  target.title=title; target.tag=tag; target.source=addSource; target.artImage=draftArtwork; target.artSource=draftArtworkSource; target.artText=(tag||title).slice(0,2).toUpperCase(); target.time=target.time||'Not started';
  if(addSource==='spotify'){
    target.url=$('#spotifyUrl').value.trim()||'https://open.spotify.com'; target.embed=extractSpotifyEmbed(target.url); target.localName=''; target.onlineName=''; target.onlinePath='';
    if(!draftArtwork) await syncSpotifyArtwork(true); target.artImage=draftArtwork; target.artSource=draftArtworkSource;
  }else if(addSource==='local'){
    const file=$('#audioFile').files[0]; if(file){ if(draftLocalUrl?.startsWith('blob:'))URL.revokeObjectURL(draftLocalUrl); draftLocalUrl=URL.createObjectURL(file); target.localName=file.name; }
    target.localUrl=draftLocalUrl; target.url=''; target.embed=''; target.onlineName=''; target.onlinePath=''; target.time=target.localUrl?'Ready to play':'Local file';
  }else{
    target.onlineName=$('#onlineName').value.trim()||'cloud-audio.mp3'; target.onlinePath=$('#onlinePath').value.trim()||`user/audio/${target.onlineName}`; target.url=''; target.embed=''; target.localUrl=''; target.localName=''; target.time='Not downloaded';
  }
  if(!existing)state.episodes.unshift(target);
  saveState(); renderAll(); closeSheets(); showToast(existing?'Episode updated':'Episode added');
}

function openPlayer(id){
  selectedEpisode=state.episodes.find(ep=>ep.id===id); if(!selectedEpisode)return;
  $('#playTitle').textContent=selectedEpisode.title; $('#playerMount').innerHTML=''; $('#openSpotify').hidden=selectedEpisode.source!=='spotify'; $('#playHere').hidden=false;
  $('#playHere').innerHTML=selectedEpisode.source==='online'?'<i data-lucide="cloud-download"></i> Download and play':'<i data-lucide="play"></i> Play in app';
  openSheet('#playSheet');
}
function playSelected(){
  if(!selectedEpisode)return;
  if(selectedEpisode.source==='spotify'){
    const embed=selectedEpisode.embed||extractSpotifyEmbed(selectedEpisode.url); if(!embed){showToast('This Spotify link cannot be embedded');return;}
    $('#playerMount').innerHTML=`<iframe class="player-frame" src="${esc(embed)}" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>`; $('#playHere').hidden=true;
  }else if(selectedEpisode.source==='local'){
    if(!selectedEpisode.localUrl){ showToast('Choose the local file again on this device'); return; }
    $('#playerMount').innerHTML=`<audio class="local-player" controls autoplay src="${esc(selectedEpisode.localUrl)}"></audio>`; $('#playHere').hidden=true;
  }else{
    $('#playHere').disabled=true; $('#playHere').innerHTML='<i data-lucide="loader-circle"></i> Downloading'; refreshIcons();
    setTimeout(()=>{ selectedEpisode.source='local'; selectedEpisode.localName=selectedEpisode.onlineName; selectedEpisode.time='Downloaded'; selectedEpisode.localUrl=''; saveState(); renderAll(); $('#playHere').disabled=false; $('#playHere').innerHTML='<i data-lucide="check"></i> Downloaded'; refreshIcons(); showToast('Online file downloaded in UI'); },900);
  }
}

function setupProfile(){
  let nickname=localStorage.getItem('geodetaNickname')||'Som'; let photo=localStorage.getItem('geodetaProfilePhoto')||''; $('#nickname').value=nickname;
  function draw(){ nickname=$('#nickname').value.trim()||'S'; localStorage.setItem('geodetaNickname',nickname); $('#profileInitial').textContent=nickname[0].toUpperCase(); $('#headerAvatar').textContent=nickname[0].toUpperCase(); $('#profilePhoto').querySelectorAll('img').forEach(img=>img.remove()); if(photo){ const img=document.createElement('img'); img.src=photo; $('#profilePhoto').prepend(img); $('#headerAvatar').innerHTML=`<img src="${photo}" alt="">`; } }
  $('#nickname').addEventListener('input',draw); $('#photoEdit').addEventListener('click',()=>$('#photoInput').click()); $('#photoInput').addEventListener('change',event=>{ const file=event.target.files[0]; if(!file)return; const reader=new FileReader(); reader.onload=()=>{photo=reader.result; localStorage.setItem('geodetaProfilePhoto',photo); draw(); showToast('Profile picture saved locally');}; reader.readAsDataURL(file); }); draw();
}
function setupLinkButton(selector,service){ const button=$(selector),key=`geodeta${service}Linked`; let linked=localStorage.getItem(key)==='true'; const draw=()=>{button.textContent=linked?'Unlink':`Link to ${service}`;button.classList.toggle('unlink',linked)}; draw(); button.addEventListener('click',()=>{linked=!linked;localStorage.setItem(key,String(linked));draw();showToast(`${service} ${linked?'linked':'unlinked'}`)}); }
function runSync(type,quiet=false){
  const status=$('#syncStatus'); status.innerHTML=`<i data-lucide="loader-circle"></i> Syncing ${type}…`; refreshIcons();
  setTimeout(()=>{
    if(type==='files'||type==='all'){
      const hasOnline=state.episodes.some(ep=>ep.source==='online'); if(!hasOnline)state.episodes.unshift({id:crypto.randomUUID(),groups:['all'],source:'online',tag:'Cloud library',title:'Synced online episode',time:'Not downloaded',progress:0,artText:'ON',artClass:'three',artImage:'',artSource:'default',onlineName:'synced-episode.mp3',onlinePath:'user/audio/synced-episode.mp3',savedAt:Date.now()}); localStorage.setItem('geodetaPendingFiles','false'); saveState(); renderAll();
    }
    status.innerHTML='<i data-lucide="check-circle-2"></i> Synced just now'; refreshIcons(); if(!quiet)showToast(type==='all'?'Everything synced':`${type[0].toUpperCase()+type.slice(1)} synced`);
  },850);
}
...
function init(){
  bindEvents(); setupProfile(); setupLinkButton('#googleLink','Google'); setupLinkButton('#spotifyLink','Spotify'); renderAll(); refreshIcons();
  setTimeout(()=>{runSync('data',true);setTimeout(()=>runSync('spotify',true),900)},450);
  setTimeout(()=>{if(localStorage.getItem('geodetaPendingFiles')==='true'&&confirm('New files were added to your account. Sync files now?'))runSync('files')},1800);
}
