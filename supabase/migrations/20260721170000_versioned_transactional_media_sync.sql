...
update public.collections set sort_order = position where sort_order = 0 and position <> 0;
update public.episodes
set sort_order = ranked.sort_order
from (
  select id,row_number() over (partition by user_id order by saved_at desc,id) - 1 as sort_order
  from public.episodes
) ranked
where public.episodes.id = ranked.id and public.episodes.sort_order = 0;

create index if not exists collections_user_active_order_idx
  on public.collections (user_id,sort_order,id) where deleted_at is null;
create index if not exists collections_parent_id_idx
  on public.collections (parent_id);
create index if not exists episodes_user_active_order_idx
  on public.episodes (user_id,sort_order,id) where deleted_at is null;
...
begin
  ...
  loop
    ...
    if v_version is null then
      raise exception 'Collection % has a newer version',v_id using errcode = '40001';
    end if;
  ...
  loop
    ...
    if v_version is null then
      raise exception 'Episode % has a newer version',v_id using errcode = '40001';
    end if;
